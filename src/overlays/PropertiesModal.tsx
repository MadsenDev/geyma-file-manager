...
const PERM_ROWS: { label: string; owner: number; group: number; other: number }[] = [
  ...
  return (
    <Modal title="Properties" onClose={onClose}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
        ...
        >
          {entry.isDir ? <Icon d={ICONS.folder} size={22} color={colors.tint} /> : <span style={{ fontFamily: t.mono, fontSize: 10, fontWeight: 700 }}>{ext}</span>}
        </div>
        <div style={{ minWidth: 0 }}>
          <div style={{ fontSize: 13.5, fontWeight: 700, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{entry.name}</div>
          <div style={{ fontSize: 11, color: t.inkFaint }}>{entry.isDir ? "Folder" : kind}</div>
        </div>
      </div>

      <Row label="Location" value={entry.path} t={t} mono />
      ...
      {error && <div style={{ fontSize: 11.5, color: t.inkFaint, marginTop: 10 }}>Permissions unavailable: {error}</div>}

      {perms && (
        <>
          <Row label="Owner" value={`${perms.owner} (${perms.uid})`} t={t} />
          <Row label="Group" value={`${perms.group} (${perms.gid})`} t={t} />
          {perms.isSymlink && <Row label="Links to" value={perms.symlinkTarget || "?"} t={t} mono />}
          ...
          >
            Permissions — {perms.mode.toString(8).padStart(3, "0")}
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr repeat(3, 52px)", gap: 6, fontSize: 11.5, alignItems: "center" }}>
            <span />
            <span style={{ textAlign: "center", color: t.inkFaint }}>Owner</span>
            <span style={{ textAlign: "center", color: t.inkFaint }}>Group</span>
            <span style={{ textAlign: "center", color: t.inkFaint }}>Other</span>
            {PERM_ROWS.map((row) => (
              <Fragment key={row.label}>
                <span style={{ color: t.inkSoft }}>{row.label}</span>
                {[row.owner, row.group, row.other].map((bit) => (
                  <span key={bit} style={{ textAlign: "center" }}>
                    <input type="checkbox" checked={(perms.mode & bit) !== 0} onChange={() => void toggleBit(bit)} />
                  </span>
                ))}
              </Fragment>
            ))}
...
function Row({ label, value, t, mono }: { label: string; value: string; t: ResolvedTheme; mono?: boolean }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between", gap: 10, padding: "3px 0", fontSize: 12 }}>
      <span style={{ color: t.inkFaint, flex: "none" }}>{label}</span>
      <span
        style={{
          color: t.ink,
