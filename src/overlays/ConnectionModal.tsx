...
const DEFAULT_PORT: Record<RemoteConnection["protocol"], number> = { sftp: 22, smb: 445 };
...
export function ConnectionModal({ initial, onClose }: ConnectionModalProps) {
  ...
  const [protocol, setProtocol] = useState<RemoteConnection["protocol"]>(initial?.protocol ?? "sftp");
  ...
  function switchProtocol(next: RemoteConnection["protocol"]) {
    ...
    if (Number(port) === DEFAULT_PORT[protocol]) setPort(String(DEFAULT_PORT[next]));
    setProtocol(next);
  }
  ...
  function handleSubmit() {
    ...
    if (!trimmedHost || !trimmedUser || (protocol === "smb" && !trimmedShare)) {
      showToast(`Fill in host, username${protocol === "smb" ? ", and share" : ""}`);
      return;
    }
  ...
  return (
    <Modal title={initial ? "Edit connection" : "New network connection"} onClose={onClose}>
      <div style={{ display: "flex", gap: 3, background: hexA(t.ink, t.isDark ? 0.14 : 0.06), borderRadius: 8, padding: 3, marginBottom: 12 }}>
      ...
      <FormField label="Label (optional)">
        <input value={label} onChange={(e) => setLabel(e.target.value)} placeholder={host || "e.g. Home NAS"} style={inputStyle(t)} />
      </FormField>
      <div style={{ display: "flex", gap: 8 }}>
        <div style={{ flex: 1 }}>
          <FormField label="Host">
            <input autoFocus value={host} onChange={(e) => setHost(e.target.value)} placeholder="nas.local or 192.168.1.10" style={inputStyle(t)} />
          </FormField>
        </div>
        <div style={{ width: 84 }}>
          <FormField label="Port">
            <input value={port} onChange={(e) => setPort(e.target.value.replace(/[^0-9]/g, ""))} style={inputStyle(t)} />
          </FormField>
        </div>
      </div>
      <FormField label="Username">
        <input value={username} onChange={(e) => setUsername(e.target.value)} style={inputStyle(t)} />
      </FormField>
      {protocol === "smb" && (
        <FormField label="Share">
          <input value={share} onChange={(e) => setShare(e.target.value)} placeholder="e.g. Media" style={inputStyle(t)} />
        </FormField>
      )}
      <FormField label={initial ? "Password (leave blank to keep using the saved one)" : "Password"}>
        <input
          type="password"
          value={password}
      ...
      <label style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 6, fontSize: 12.5, color: t.inkSoft, cursor: "pointer" }}>
        <input type="checkbox" checked={remember} onChange={(e) => setRemember(e.target.checked)} />
        Remember password (stored in your OS keyring)
      </label>
...
function inputStyle(t: ReturnType<typeof useTheme>): React.CSSProperties {
  return { width: "100%", height: 32, border: `1px solid ${t.border}`, borderRadius: 8, padding: "0 10px", fontSize: 12.5, background: t.main, color: t.ink, fontFamily: "inherit" };
}
