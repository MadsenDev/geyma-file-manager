...
import { LAYOUT_PRESETS, layoutSignature, mergeLayout, type Layout } from "../state/layout";
...
function PresetThumb({ layout }: { layout: Layout }) {
  ...
  return (
    <div style={{ width: "100%", height: 52, display: "flex", flexDirection: "column", gap: 3, padding: 4, boxSizing: "border-box", background: t.bg, border: `1px solid ${t.border}`, borderRadius: 6 }}>
      {has("top") && <div style={{ height: 8, flex: "none", background: soft, borderRadius: 2 }} />}
      <div style={{ flex: 1, display: "flex", gap: 3, minHeight: 0 }}>
        {has("left") && <div style={{ width: "22%", background: soft, borderRadius: 2 }} />}
        <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column", gap: 3 }}>
          <div style={{ flex: 1, background: acc, borderRadius: 2 }} />
          {has("center2") && <div style={{ flex: 1, background: acc, borderRadius: 2 }} />}
        </div>
        {has("right") && <div style={{ width: "26%", background: soft, borderRadius: 2 }} />}
      </div>
      {has("bottom") && <div style={{ height: 7, flex: "none", background: soft, borderRadius: 2 }} />}
    </div>
  );
}
          ...
          {LAYOUT_PRESETS.map((p) => {
            ...
            return (
              ...
              >
                ...
                <span style={{ fontSize: 11.5, fontWeight: active ? 700 : 600, color: active ? t.accent : t.inkSoft, textAlign: "left", paddingLeft: 2 }}>{p.name}</span>
              </button>
            );
          })}
        </div>
