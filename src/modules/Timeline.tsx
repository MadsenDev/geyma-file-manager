...
function groupLabel(ms: number): string {
  ...
  if (daysAgo < 7) return tr("ui.timeline.this_week");
  return "EARLIER";
}
export function Timeline() {
  ...
  return (
    <div>
      <div style={panelTitleStyle(t)}>{tr("ui.timeline.timeline")}</div>
      <div
        style={{
          padding: "0 8px 8px",
        }}
      >
          ...
          >
            ...
            {evs.map((ev) => (
              <button
                ...
                onContextMenu={(event) =>
                  openReferencedPathMenu(event, ev.path)
                }
                style={{
                  display: "flex",
                  alignItems: "center",
              ...
              >
                ...
                >
                  {new Date(ev.whenMs).toLocaleTimeString([], {
                    hour: "2-digit",
                    minute: "2-digit",
                  })}
                </span>
        ...
        {feed.length === 0 && (
          ...
          >
            {tr("ui.timeline.nothing_recorded_yet")}
          </div>
        )}
      </div>
    </div>
