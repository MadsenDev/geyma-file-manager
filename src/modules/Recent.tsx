...
export function Recent() {
  ...
  return (
    <div>
      <div style={panelTitleStyle(t)}>Recent activity</div>
        ...
        {feed.slice(0, count).map((ev) => (
          ...
          >
            ...
            <span style={{ flex: 1, fontSize: 12, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
              <b style={{ color: t.ink }}>{ev.action}</b> <span style={{ color: t.inkSoft }}>{ev.path.split("/").pop()}</span>
            </span>
            <span style={{ fontFamily: t.mono, fontSize: 9, color: t.inkFaint, flex: "none" }}>{formatAgo(ev.whenMs)}</span>
          </button>
        ))}
        {feed.length === 0 && <div style={{ padding: "6px 4px", fontSize: 11.5, color: t.inkFaint }}>No activity yet — file changes will appear here.</div>}
      </div>
    </div>
  );
}
