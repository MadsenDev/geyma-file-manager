...
export function Location() {
  ...
  const label = path === home ? "Home" : rel;
  const parts = label.split("/").filter(Boolean);
  const crumbs = [{ name: "Home", full: home }, ...parts.map((_, i) => ({ name: parts[i], full: home + "/" + parts.slice(0, i + 1).join("/") }))];
  const realCrumbs = path === home ? [{ name: "Home", full: home }] : crumbs;

  return (
    <div style={{ display: "flex", alignItems: "center", gap: 3, minWidth: 0, flex: 1, overflow: "hidden" }}>
      {realCrumbs.map((c, i) => (
        <span key={c.full} style={{ display: "flex", alignItems: "center", gap: 3, minWidth: 0 }}>
      ...
      <div style={{ width: 1, height: 16, background: hexA(t.ink, 0.08), margin: "0 4px", flex: "none" }} />
    </div>
  );
}
